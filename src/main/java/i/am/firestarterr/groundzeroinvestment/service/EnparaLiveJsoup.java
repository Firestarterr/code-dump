...
public class EnparaLiveJsoup {
    ...
    private void prettyPrintValues(Double sell, Double buy, String kurAdi) {
        ...
        System.out.println(kurAdi +
                BigDecimal.valueOf(buy).setScale(6, RoundingMode.HALF_UP) +
                " / " +
                kurAdi +
                BigDecimal.valueOf(sell).setScale(6, RoundingMode.HALF_UP) +
                "\t\t\t : \t makas: " +
                BigDecimal.valueOf(diff).setScale(6, RoundingMode.HALF_UP) +
                "\t makas bölü alım: " +
                BigDecimal.valueOf(diffOverBuy).setScale(6, RoundingMode.HALF_UP)
        );
    }
}